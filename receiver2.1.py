...
with sd.InputStream(callback=audio_callback, channels=1, samplerate=SAMPLE_RATE, blocksize=int(SAMPLE_RATE * DURATION)):
    ...
    while True:
        try:
            ...
            # Grafiği güncelle
            ax.clear()
            ax.plot(filtered_freqs, fft_magnitudes, color='blue', label="Spektrum")
            ax.scatter(dominant_freq, dominant_amp, color='red', s=50, label=f"Dominant: {dominant_freq:.1f} Hz")
            ...
            # Dominant frekansı terminale yazdır
